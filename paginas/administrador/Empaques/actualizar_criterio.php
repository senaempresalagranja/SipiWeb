...
$query="SELECT `Id_Empaque`, `Empaque` FROM `empaques` WHERE `Id_Empaque`='$criterio' ";
    ...
    <div class="container-fluid xyz">
      <div class="row">
        <div class="col-xs-10 col-sm-12 col-md-12 col-lg-12">
          <h1 >Actualizar Empaque  "<?php echo " $fila[1] "; ?>" <i class=" icon icon-stack"></i> </h1>
        </div>
      </div>
  <div class="row">
...
function validar_criterio() {
  ...
  if(criterio==null  || criterio.length==0 ){
    ...
    return false;
	}else if (isNaN(criterio)==false) {
    var criterio=document.getElementById('criterio').style.border="2px solid red";
    return false;
		}else if (criterio.length>50) {
...
$("#contenedor").load("actualizar.php",{criterio:criterio,id_criterio:id_criterio})
    ...
    swal('Error','Algunos Campos Estan Vacios o Incorrectos')
  }
